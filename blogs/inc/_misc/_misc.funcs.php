 ...
 * @param boolean Use ambiguous chars? (1 (one) vs l (L) vs I (i)).
 *                Set this to false for data a user has to read/type (passwords).
 * @return string key
 */
function generate_random_key( $length = NULL, $use_ambiguous = true )
{
	// fp> NOTE: do not include any characters that would make autogenerated passwords ambiguous
	// 1 (one) vs l (L) vs I (i)
	// O (letter) vs 0 (digit)
	static $keychars_no_ambiguous = 'abcdefghijkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789';
	...
	$chars = $use_ambiguous ? $keychars : $keychars_no_ambiguous;

	$key = '';
	$rnd_max = strlen($chars) - 1;

	for( $i = 0; $i < $length; $i++ )
	{
		$key .= $chars{mt_rand(0, $rnd_max )}; // get a random character out of $chars
	}
