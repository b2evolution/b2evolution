...
<body bgcolor="#EEEEEE" text="#000000" link="#0000FF" alink="#FF0000" vlink="#7E0089">
	...
	if ( ! isset ( $_POST['wp_db'] ) ) { ?>
		...
		<p><strong><font color="#FF0000">Warning!!</strong> Your existing b2evolution posts, categories, comments and users (except admin) will be removed if you run this script. Make sure you have a backup before you proceed.</font></p>
		...
		<table>
			...
			<td><INPUT type="text" name="b2evo_db"></td>
			</tr>

			<td>b2evolution table prefix</td>
			<td><INPUT type="text" name="b2evo_prefix" value="evo_"></td>
			</tr>
			<tr>
	...
	<?php // The form has been posted; do the conversion
		...
		else
		{
			...
			// Get the categories
			echo '<h2>Trying to import categories:</h2>';
			$cats = array ();
			...
			while ( $row = mysql_fetch_array ($res, MYSQL_ASSOC) )
			{
				...
				$query2 = 'SELECT * FROM ' . $wp . 'term_taxonomy WHERE term_id=' . $row['term_id'] . ';';
			...
			while ( $row = mysql_fetch_array ($res, MYSQL_ASSOC) )
			{
				...
				// Now to get the cats for each post. This is a bit tricky since we have to avoid 'tags' in the result
				$j = 0;
				$posts[$i]['cats'] = array ();
				
				$query2 = 'SELECT * FROM ' . $wp . 'term_relationships WHERE object_id=' . $row['ID'] . '; ';
				$res2 = mysql_query ($query2);
				if (! $res2)
					die ('Query 2 failed. Please check your WordPress installation.');
				...
				{
					$query3 = 'SELECT * FROM ' . $wp . 'term_taxonomy WHERE term_taxonomy_id=' . $row2['term_taxonomy_id'] . ';';
					$res3 = mysql_query ($query3);
					if (! $res3 )
						die ('Query 3 failed. Please check your WordPress installation.');
					$row3 = mysql_fetch_array ($res3, MYSQL_ASSOC);
					if (strcmp ($row3['taxonomy'], 'category') == 0)
					{
			...
			{
				// insert the post categories first into the postcats table
			...
			foreach ($comments as $comment)
			{
				...
				$query = 'INSERT INTO ' . $b2 . 'comments (comment_ID, comment_post_ID, comment_type, comment_status, comment_author_ID, comment_author, comment_author_email, comment_author_url, comment_author_IP, comment_date, comment_content, comment_allow_msgform) VALUES ("'. $comment['comment_id'] . '", "' . $comment['post_id'] . '", "' . $comment['type'] . '", "' . $comment['status'] . '", "' . $comment['author_id'] . '", "' . $comment['author'] . '", "' . $comment['email'] . '", "' . $comment['url'] . '", "' . $comment['ip'] . '", "' . $comment['date'] . '", "' . $ccontent . '", "1");';
