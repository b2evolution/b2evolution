...
class Blog extends DataObject
{
  ...
  /**
	 * get path to cahce for this blog
	 */
	function cache_get_ads_collcache_path()
	{
		global $cache_path;
		return $cache_path.'c'.$this->ID.'/';
	}
  ...
  /**
	 ...
	 */
	function cache_get_af_filecache_path()
	{
		global $Debuglog;
		global $ReqHost, $ReqURI;

		if( !empty($url) )
		{ // We want the cache for a specific UR

		}

		// We want the Ucache for the current URL
		if( empty( $this->cache_filepath ) )
		{
			...
			// echo $ReqAbsUrl;
 			...
 			$this->cache_filepath = $this->cache_gen_filecahe_path( $ReqAbsUrl );
  ...
  /**
	 *
	 */
	function cache_gen_filecahe_path( $url )
	{
		...
		$ads_collcache_path = $this->cache_get_ads_collcache_path();

		return $ads_collcache_path.$url_hash.'.page';
	}
    ...
    // What would be the cache file for the current URL?
		$af_cache_file = $this->cache_gen_filecahe_path( $url );
  ...
  /**
	...
	function cache_check()
	{
		...
		if( $this->cache_retrieve() )
		{ // We could retrieve:
			return true;
		}


		$this->cache_is_collecting = true;

		$Debuglog->add( 'Collecting started', 'cache' );

		ob_start( array( & $this, 'cache_output_handler'), 2000 );
   ...
   *
	...
	function cache_retrieve()
	{
		...
		// What would be the cache file for the current URL?
		$af_cache_file = $this->cache_get_af_filecache_path();
  ...
  /**
	 ...
	 */
	function cache_output_handler( $buffer )
	{
  ...
  /**
	 ...
	 */
	function cache_abort_collect()
	{
		global $Debuglog;

		if( ! $this->cache_is_collecting )
		...
		$this->cache_is_collecting = false;
	}
