...
$trans['ja_JP'] = array(
	...
	'The page you\'re looking at' => "あなたが見ているページは",
	...
	'Back to new blog form' => "新規blogのフォームに戻る",
	...
	'You should check the permission for [%s]. See <a %s>online manual on file permissions</a>.' => "[%s] の権限を確認するべきです。<a %s>ファイル権限についてのオンラインマニュアル</a> を参照してください。",
	...
	'You must not create empty locales!' => "空の言語・地域設定を作ってはいけません!",
	...
	'You have no permission to edit other users or groups!' => "あなたは他のユーザやグループを編集する権限がありません!",
	'You cannot change the demouser profile in demo mode!' => "デモモード中に demouser のプロフィールを変更することはできません!",
	'You are only allowed to update yourself!' => "あなたは自分自身について更新することだけ許可されています!",
	'You must provide an unique Login!' => "ユニークなログインを提供しなければなりません!",
	'User level must be between %d and %d.' => "ユーザーレベルは %d と %d の間にする必要があります。",
	'The login already exists. Please <a %s>edit this login</a> instead of overwriting it this way.' => "そのログインはすでに存在しています。この方法で上書きする代わりに <a %s>このログインを編集</a> してください。",
	...
	'Couldn\'t change %s\'s level.' => "%s のレベルを変更できません。",
	...
	'The group name already exists. Please <a %s>edit this group</a> instead of overwriting it this way.' => "そのグループ名はすでに存在しています。この方法で上書きする代わりに <a %s>このグループを編集</a> してください。",
	...
	'Non members' => "非メンバー",
	'Warning! You are about to withdraw your permission to edit this blog!\\nYou won\\\'t have access to its properties if you do that!' => "警告! あなたはこのblogを編集する自身の権限を取り消そうとしています!\\nこれを行った場合、このblogのプロパティにアクセスできなくなります!",
	...
	'Permamnent link to this comment' => "このコメントへの永続的リンク",
	...
	'Create new Locale' => "新しい言語・地域設定の作成",
	'Edit Locale' => "言語・地域設定の編集",
	...
	'Enabled' => "有効",
	'should the locale be available?' => "その言語・地域設定は有効?",
	...
	'Lang file' => "言語ファイル",
	'the message file being used, from <code>locales</code> subdirectory' => "<code>locales</code> サブディレクトリから、使用されるメッセージ ファイルです。",
	'Priority' => "優先度",
	'1 is highest. It\'s use is for locale autodetect (from HTTP_ACCEPT_LANGUAGE header), when we have more than locale with the requested language. It also affects the order in which locales are shown in dropdown boxes etc.' => "1 が一番高い。要求された言語の言語・地域設定が複数ある時に、言語・地域設定の自動判別に用いられます ( HTTP_ACCEPT_LANGUAGE ヘッダから)。これはまた、ドロップダウン ボックスなどに表示される言語・地域設定の順番にも影響します。",
	'Create' => "作成",
	'This will replace locale \\\'%s\\\'. Ok?' => "言語・地域設定 \\'%s\\' を置き換えます。Ok?",
	'We\'ll use the flag out of subdirectories from <code>%s</code>, where the filename is equal to the language part of the locale (characters 4-5; file extension is gif).' => "<code>%s</code> のサブディレクトリから、言語・地域設定の地域部分と同じファイル名の旗を使用します (4-5文字目; 拡張子は gif)。",
	...
	'Default locale. Overwritten from HTTP_ACCEPT_LANGUAGE header, user locale or blog locale (in this order).' => "デフォルトの言語・地域設定です。HTTP_ACCEPT_LANGUAGE ヘッダや、ユーザ言語・地域設定、blog言語・地域設定によって (この順番で) 上書きされます。",
	...
	'New user...' => "新規ユーザ...",
	'New group...' => "新規グループ...",
	'The default skin [%s] does not exist. It must be properly set in the blog stub file [%s]. Contact the <a %s>webmaster</a>...' => "デフォルトのスキン [%s] が存在しません。blogのスタブ ファイル [%s] に適切に設定されていなければなりません。<a %s>webmaster</a> に連絡してください...",
	...
	'Response:' => "応答:",
	'wrong login/password.' => "login名 か password が間違っています",
	'setcookie %s failed!' => "setcookie %s が失敗しました!",
	'login/password no longer valid.' => "login名/password がもう有効ではありません",
	...
	'Could not load original conf file [%s]. Is it missing?' => "元の設定ファイル [%s] が読み込めません。紛失していませんか?",
	...
	'Your config file has not been edited yet. You can do this by filling in the form below.' => "設定ファイルはまだ編集されていません。以下のフォームに入力することで、編集を行うことができます。",
	'This is the minimum info we need to set up b2evolution on this server:' => "このサーバで b2evolution を設置するために必要な最低限の情報:",
	'Database you want to install into (These settings should be provided by your host)' => "インストールしたいデータベース (これらの設定はホストから提供されているはずです)",
	...
	'<p>If you have installed b2evolution tables before and wish to start anew, you must delete the b2evolution tables before you can start a new installation. b2evolution can delete its own tables for you, but for obvious security reasons, this feature is disabled by default.</p>' => "<p>以前に b2evolution テーブルをインストールしていて、新規に始めたい場合は、新規インストールを始める前に、以前の b2evolution テーブルを削除する必要があります。 b2evolution は自身のテーブルを削除することができますが、明らかなセキュリティ上の理由から、この機能はデフォルトでは使用不能にされています。</p>",
	'Config file recap...' => "設定ファイルの要約...",
	...
	'<p>For security reasons, the reset feature is disabled by default.</p>' => "<p>セキュリティ上の理由から、デフォルトではリセット機能は使用不能になっています。</p>",
	'Reset done!' => "リセットしました!",
	...
	'<p>By default, b2evolution comes with 4 blogs, named \'Blog All\', \'Blog A\', \'Blog B\' and \'Linkblog\'.</p>\n\n<p>Some of these blogs have a special role. Read about it on the corresponding page.</p>\n\n<p>You can create additional blogs or delete unwanted blogs from the blogs admin.</p>' => "<p>デフォルトでは、b2evolution は 'Blog All', 'Blog A', 'Blog B', 'Linkblog' という名前の4つのblogを提供します。</p>\n\n<p>これらのblogのいくつかには特別な役割があります。それについては対応するページを読んでください。</p>\n\n<p>blog管理画面から、追加のblogを作ることができますし、不要なblogを削除することもできます。</p>",
