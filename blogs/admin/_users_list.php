...
<table class="thin">
	<tr>
		...
		<?php if( $current_User->check_perm( 'users', 'edit' ) )
					{
						echo '<th>'.T_('Edit').'</th>';
					}
		?>
	...
	if( count($userlist) ) foreach( $userlist as $row )
	{	// For each line (can be a user/group or just an empty group)
		...
		{	// We just entered a new group!
			?>
			<tr class="group">
				<td colspan="<?php echo (7 + (int)$current_User->check_perm( 'users', 'edit' ))?>">
...
{ // create new user link
	echo '<div align="center"><a href="?action=newuser">'.T_('Create a new user').'</a></div>';
}
...
if( $current_User->check_perm( 'users', 'edit' ) )
{
		...
		<?php echo T_('To delete an user, bring his/her level to zero, then click on the red cross.') ?>
		<br />
