...
function get_msgform_contact_methods( $recipient_User = NULL )
{
	...
	{	// If current User is logged in and recipient User is defined:
		if( $current_User->get_msgform_possibility( $recipient_User, 'PM' ) )
		...
		if( $recipient_User->get_msgform_possibility( NULL, 'email' ) )
		{	// Suggest Email method only if recipient user allows to receive emails:
