...
function md_import( $folder_path, $source_type, $source_folder_zip_name )
{
	...
	foreach( $files as $file_path )
	{
		...
		if( $import_type != 'upgrade' ||
		    ! ( $Item = & md_get_Item( $item_slug, $category_ID ) ) )
...
/**
 * Get Item by slug in given Category
 *
 * @param string Item slug
 * @param string Category ID
 * @return object|NULL Item object
 */
function & md_get_Item( $item_slug, $category_ID )
{
	$ItemCache = & get_ItemCache();

	if( ( $Item = & $ItemCache->get_by_urltitle( $item_slug, false, false ) ) &&
	    $Item->get( 'main_cat_ID' ) == $category_ID )
	{	// Item is found by requested slug in the given Category:
		return $Item;
	}

	// Try to find Item by slug with suffix like "-123" in the requested Category:
	global $DB;
	$item_slug_base = preg_replace( '/-\d+$/', '', $item_slug );
	$SQL = new SQL( 'Find Item by slug base "'.$item_slug_base.'/" in the category #'.$category_ID );
	$SQL->SELECT( 'post_ID' );
	$SQL->FROM( 'T_items__item' );
	$SQL->WHERE( 'post_main_cat_ID = '.$DB->quote( $category_ID ) );
	$SQL->WHERE_and( 'post_urltitle REGEXP '.$DB->quote( '^('.$item_slug_base.')(-[0-9]+)?$' ) );
	$SQL->ORDER_BY( 'post_urltitle' );
	$SQL->LIMIT( '1' );
