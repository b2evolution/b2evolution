...
function dbm_recreate_itemslugs()
{
	...
	// Find all item canonical slugs with old titles:
	$SQL = new SQL( 'Get old item canonical slugs "title-[0-9]"' );
	$SQL->SELECT( 'post_ID, post_title' );
	$SQL->FROM( 'T_slug' );
	$SQL->FROM_add( 'INNER JOIN T_items__item ON slug_ID = post_canonical_slug_ID' );
	$SQL->WHERE( 'post_title != ""' );
	$SQL->WHERE_and( 'slug_type = "item"' );
	$SQL->WHERE_and( 'slug_title = "title" OR slug_title REGEXP "^title(-[0-9]+)?$"' );
	$old_slugs = $DB->get_assoc( $SQL->get(), $SQL->title );

	$new_slugs_count = 0;
	foreach( $old_slugs as $post_ID => $post_title )
	{
		$item_Slug = new Slug();
		$item_Slug->set( 'type', 'item' );
		$item_Slug->set( 'itm_ID', $post_ID );
		$item_Slug->set( 'title', urltitle_validate( $post_title, $post_title, $post_ID, false, $item_Slug->dbprefix.'title', $item_Slug->dbprefix.'itm_ID', $item_Slug->dbtablename ) );
		if( $item_Slug->dbinsert() )
		{	// Update old item canonical slug to new autogenerated from title:
			$DB->query( 'UPDATE T_items__item
				  SET post_canonical_slug_ID = '.$item_Slug->ID.'
				WHERE post_ID = '.$post_ID );
			$new_slugs_count++;
		}
	}

	$Messages->add( sprintf( 'Created %d new URL slugs.', $new_slugs_count ), 'success' );
