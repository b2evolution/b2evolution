...
if( count( $goal_rows ) && count( $hitgroup_array ) )
{
	...
	$chart['link_data']['url'] = $admin_url.'?ctrl=stats&tab=goals&tab3=hits'.$coll_group_params.'&datestartinput=$date$&datestopinput=$date$&goal_name=$param1$';
...
$Table->filter_area = array(
	...
	'presets' => array(
		'all' => array( T_('All'), '?ctrl=goals&amp;tab3=stats'.$coll_group_params ),
		'final' => array( T_('Final'), '?ctrl=goals&amp;tab3=stats&amp;final=1'.$coll_group_params ),
		)
	);
...
{ // Display table
	...
	foreach( $hitgroup_array as $day => $hitday_array )
	{
		...
		{ // For each named goal, display count:
			...
			{
				echo '<a href="'.$admin_url.'?ctrl=stats&amp;tab=goals&amp;tab3=hits&amp;datestartinput='.$date_param.'&amp;datestopinput='.$date_param.$coll_group_params.'&amp;goal_name='.rawurlencode( $goal_row->goal_name ).'">'.$hitday_array[ $goal_row->goal_ID ].'</a>';
