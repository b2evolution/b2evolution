...
class Chapter extends DataObject
{
	...
	/**
	 * New slugs, separated by comma, which should be added for the Chapter, see @update_slugs()
	 * First slug will be used as canonical.
	 *
	 * @var string
	 */
	var $new_slugs;
	...
	function load_from_request()
	{
		...
		// Set slugs:
		$this->new_slugs = param( 'cat_slugs', 'string' );
	...
	function get_permanent_url( $link_type = NULL, $blogurl = NULL, $paged = 1, $chapter_posts_per_page = NULL, $glue = '&amp;' )
	{
		...
		switch( $link_type )
		{
			...
			case 'subchap':
				...
				{
					$r = url_add_tail( $blogurl, '/'.$category_prefix.'/'.$this->get_canonical_slug().'/' );
				}
				else
				{
					$r = url_add_tail( $blogurl, '/'.$this->get_canonical_slug().'/' );
				}
				break;
	...
	function dbinsert()
	{
		...
		{	// The chapter was inserted successful
			...
			{	// Use autogenerated slug title if new slugs have not been entered on the submitted form:
				$this->new_slugs = urltitle_validate( '', $this->get( 'name' ), $this->ID, false, 'slug_title', 'slug_cat_ID', 'T_slug' );
			}
	 ...
	 *
	 * @param boolean TRUE to update slugs
	 * @return boolean true on success
	 */
	function dbupdate( $update_slugs = true )
	{
		global $DB;
	...
	{
		// Get all slugs from cache or initialize them into cache for further using:
		$slugs = $this->get_slugs();

		if( ! isset( $slugs[0] ) )
		{	// If this Chapter has no canonical slug by some unknown reason:
			debug_die( 'Chapter #'.$this->ID.' has no canonical slug!' );
		}
	...
	function get_slugs()
	{
		...
		if( $this->slugs === NULL )
		{	// Get slugs from DB once and store in cache var:
			global $DB;

			$SQL = new SQL( 'Get slugs of the Chapter #'.$this->ID );
			$SQL->SELECT( 'slug_title, IF( slug_ID = '.intval( $this->get( 'canonical_slug_ID' ) ).', 0, slug_ID ) AS slug_order_num' );
			$SQL->FROM( 'T_slug' );
			$SQL->WHERE( 'slug_cat_ID = '.$DB->quote( $this->ID ) );
			$SQL->WHERE_and( 'slug_type = "cat"' );
			$SQL->ORDER_BY( 'slug_order_num' );

			// Put slugs in cache var:
	...
	/**
	 * Update slugs or create new if it doesn't exist yet
	 * Slugs should be stored in var $this->new_slugs separated by comma
	 */
	function update_slugs()
	{
		...
		foreach( $new_slugs as $new_slug_title )
		{
			...
			$cat_Slug = & $SlugCache->get_by_name( $new_slug_title, false, false );

			if( ! $cat_Slug || $cat_Slug->get( 'cat_ID' ) != $this->ID )
			{	// Create new slug if it doesn't exist in DB or slug is used for other object(Chapter or Item):
				...
				$cat_Slug->set( 'title', urltitle_validate( $new_slug_title, $new_slug_title, $this->ID, false, $cat_Slug->dbprefix.'title', $cat_Slug->dbprefix.'cat_ID', $cat_Slug->dbtablename ) );
				$cat_Slug->dbinsert();
			}
