...
class display_item_Widget extends ComponentWidget
{
	...
	function display( $params )
	{
		...
		{	// Item is not found by ID and slug or it is not a post or page:
			...
			display_rendering_error( sprintf( T_('The referenced Item (%s) is not a Post or Standalone Page.'), utf8_trim( $wrong_item_info ) ) );
		}
		elseif( ! $widget_Item->can_be_displayed() )
		{	// Current user has no permission to view item with such status:
			display_rendering_error( sprintf( T_('Post/Page "%s" cannot be included because you have no permission.'), '#'.$widget_Item->ID.' '.$widget_Item->get( 'urltitle' ) ) );
		}
		...
		{	// Display error if the requested content block item cannot be used in this place:
			display_rendering_error( sprintf( T_('Post/Page "%s" cannot be included here. It must be in the same collection or have the same owner.'), '#'.$widget_Item->ID.' '.$widget_Item->get( 'urltitle' ) ) );
		}
