 ...
 * @package evocore
 */
class mustread_menu_link_Widget extends generic_menu_link_Widget
{
	var $icon = 'flag';
	...
	function __construct( $db_row = NULL )
	{
		...
		parent::__construct( $db_row, 'core', 'mustread_menu_link' );
	}
	...
	{
		return get_manual_url( 'mustread-items-menu-link-widget' );
	}
	...
	function get_name()
	{
		return T_('Must Read Items Menu link or button');
	}
	...
	function get_param_definitions( $params )
	{
		$r = array_merge( array(
				...
				'show_badge' => array(
					'label' => T_( 'Show Badge' ),
					'note' => T_('Show a badge with the count of must read items.'),
					'type' => 'checkbox',
					'defaultvalue' => true,
				),
				'hide_empty' => array(
	...
	function display( $params )
	{
		...
		if( ! $current_Blog->get_setting( 'track_unread_content' ) )
		{	// Don't use this widget when current collection doesn't use a tracking of unread content:
			$this->display_debug_message( 'Hidden(No tracking)' );
			return false;
		}
		...
		if( $this->disp_params['hide_empty'] && $current_Blog->get_must_read_items_count() == 0 )
		{	// Hide this menu if current user has no must read posts yet:
			$this->display_debug_message( 'Hidden(No must read posts)' );
			return false;
		}
