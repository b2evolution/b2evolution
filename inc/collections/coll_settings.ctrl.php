...
if( $action == 'dashboard' )
{
	...
	if ( $blog )
	{
		...
		echo '<div class="row">';
			...
			echo '</div>';
			echo '<h2 class="col-xs-12 col-sm-10 col-sm-pull-2 page-title">'.get_coll_fav_icon( $Blog->ID, array( 'class' => 'coll-fav' ) ).'&nbsp;'.$Blog->dget( 'name' ).' <span style="font-size: 0.6em;">('.T_('Collection').' #'.$Blog->ID.')</span>'.'</h2>';
		echo '</div>';
