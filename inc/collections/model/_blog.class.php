...
class Blog extends DataObject
{
	...
	/**
	 * Include widget data record to array in order to insert all included records in DB by single SQL query later
	 *
	 * @param string Container name
	 * @param string Type: 'core', 'plugin'
	 * @param string Code
	 * @param integer Order
	 * @param array|string|NULL Widget params: NULL - for default params, array or string as string serialized array
	 * @param integer 1 - enabled, 0 - disabled
	 */
	function include_widget( $container_name, $code, $type, $order, $params = NULL, $enabled = 1 )
	{
		global $DB;
		...
		$this->widgets_insert_records[] = '( '
			.$this->ID.', ' // Collection ID
			.$DB->quote( $container_name ).', ' // Container name
			.$order.', ' // Widget order inside container
			.$enabled.', ' // Enabled?
			.$DB->quote( $type ).', ' // Type: 'core', 'plugin'
			.$DB->quote( $code ).', ' // Code
			.$DB->quote( $params ).' )'; // Additional params
	}
	...
	function setup_default_widgets( $context = array() )
	{
		...
		$this->widgets_insert_records = array();
		...
		/* Header */
		$this->include_widget( 'Header', 'coll_title', 'core', 1 );
		$this->include_widget( 'Header', 'coll_tagline', 'core', 2 );
		...
		{ // Don't add widgets to Menu container for Main collections
			// Home page
			$this->include_widget( 'Menu', 'menu_link', 'core', 5, array( 'link_type' => 'home' ) );
			if( $this->ID == $blog_b_ID )
			{ // Recent Posts
				$this->include_widget( 'Menu', 'menu_link', 'core', 10, array( 'link_type' => 'recentposts', 'link_text' => T_('News') ) );
			}
			if( $kind == 'forum' )
			{ // Latest Topics and Replies ONLY for forum
				$this->include_widget( 'Menu', 'menu_link', 'core', 13, array( 'link_type' => 'recentposts', 'link_text' => T_('Latest topics') ) );
				$this->include_widget( 'Menu', 'menu_link', 'core', 15, array( 'link_type' => 'latestcomments', 'link_text' => T_('Latest replies') ) );
			}
			if( $kind == 'manual' )
			{ // Latest Topics and Replies ONLY for forum
				$this->include_widget( 'Menu', 'menu_link', 'core', 13, array( 'link_type' => 'recentposts', 'link_text' => T_('Latest pages') ) );
				$this->include_widget( 'Menu', 'menu_link', 'core', 15, array( 'link_type' => 'latestcomments', 'link_text' => T_('Latest comments') ) );
			}
			if( $kind == 'forum' || $kind == 'manual' )
			{	// Add menu with flagged items:
				$this->include_widget( 'Menu', 'flag_menu_link', 'core', 17, array( 'link_text' => ( $kind == 'forum' ) ? T_('Flagged topics') : T_('Flagged pages') ) );
			}
			if( $kind == 'photo' )
			{ // Add menu with Photo index
				$this->include_widget( 'Menu', 'menu_link', 'core', 18, array( 'link_type' => 'mediaidx', 'link_text' => T_('Index') ) );
			}
			if( $kind == 'forum' )
			{ // Add menu with User Directory
				$this->include_widget( 'Menu', 'menu_link', 'core', 20, array( 'link_type' => 'users' ) );
			}
			// Pages list:
			$this->include_widget( 'Menu', 'coll_page_list', 'core', 25 );
			if( $kind == 'forum' )
			{ // My Profile
				$this->include_widget( 'Menu', 'menu_link', 'core', 30, array( 'link_type' => 'myprofile' ), 0 );
			}
			if( $kind == 'std' )
			{ // Categories
				$this->include_widget( 'Menu', 'menu_link', 'core', 33, array( 'link_type' => 'catdir' ) );
				// Archives
				$this->include_widget( 'Menu', 'menu_link', 'core', 35, array( 'link_type' => 'arcdir' ) );
				// Latest comments
				$this->include_widget( 'Menu', 'menu_link', 'core', 37, array( 'link_type' => 'latestcomments' ) );
			}
			$this->include_widget( 'Menu', 'msg_menu_link', 'core', 50, array( 'link_type' => 'messages' ), 0 );
			$this->include_widget( 'Menu', 'msg_menu_link', 'core', 60, array( 'link_type' => 'contacts', 'show_badge' => 0 ), 0 );
			$this->include_widget( 'Menu', 'menu_link', 'core', 70, array( 'link_type' => 'login' ), 0 );
			if( $kind == 'forum' )
			{ // Register
				$this->include_widget( 'Menu', 'menu_link', 'core', 80, array( 'link_type' => 'register' ) );
			}
		}
		...
		{
			$this->include_widget( 'Item Single Header', 'item_info_line', 'core', 10, 'a:14:{s:5:"title";s:0:"";s:9:"flag_icon";i:1;s:14:"permalink_icon";i:0;s:13:"before_author";s:10:"started_by";s:11:"date_format";s:8:"extended";s:9:"post_time";i:1;s:12:"last_touched";i:1;s:8:"category";i:0;s:9:"edit_link";i:0;s:16:"widget_css_class";s:0:"";s:9:"widget_ID";s:0:"";s:16:"allow_blockcache";i:0;s:11:"time_format";s:4:"none";s:12:"display_date";s:12:"date_created";}' );
			$this->include_widget( 'Item Single Header', 'item_tags', 'core', 20 );
			$this->include_widget( 'Item Single Header', 'item_seen_by', 'core', 30 );
		}
		else
		{
			$this->include_widget( 'Item Single Header', 'item_info_line', 'core', 10 );
		}

		/* Item Single */
		$this->include_widget( 'Item Single', 'item_content', 'core', 10 );
		$this->include_widget( 'Item Single', 'item_attachments', 'core', 15 );
		if( $this->ID != $blog_a_ID && ( empty( $events_blog_ID ) || $this->ID != $events_blog_ID ) && ! in_array( $kind, array( 'forum', 'group' ) ) )
		{ // Item Tags
			$this->include_widget( 'Item Single', 'item_tags', 'core', 20 );
		}
		if( $this->ID == $blog_b_ID )
		{ // About Author
			$this->include_widget( 'Item Single', 'item_about_author', 'core', 25 );
		}
		if( ( $this->ID == $blog_a_ID || ( ! empty( $events_blog_ID ) && $this->ID == $events_blog_ID ) ) && $install_test_features )
		{ // Google Maps
			$this->include_widget( 'Item Single', 'evo_Gmaps', 'plugin', 30 );
		}
		if( $this->ID == $blog_a_ID || $kind == 'manual' )
		{ // Small Print
			$this->include_widget( 'Item Single', 'item_small_print', 'core', 40, array( 'format' => ( $this->ID == $blog_a_ID ? 'standard' : 'revision' ) ) );
		}
		if( ! in_array( $kind, array( 'forum', 'group' ) ) )
		{ // Seen by
			$this->include_widget( 'Item Single', 'item_seen_by', 'core', 50 );
		}
		if( $kind != 'forum' )
		{	// Item voting panel:
			$this->include_widget( 'Item Single', 'item_vote', 'core', 60 );
		}
		...
		if( $kind == 'manual' )
		{
			...
			$this->include_widget( 'Sidebar', 'coll_search_form', 'core', 10, $search_form_params );
			$this->include_widget( 'Sidebar', 'content_hierarchy', 'core', 20 );
		}
		else
		{
			if( $install_test_features )
			{
				...
				{ // Current filters widget
					$this->include_widget( 'Sidebar', 'coll_current_filters', 'core', 5 );
				}
				// User login widget
				$this->include_widget( 'Sidebar', 'user_login', 'core', 10 );
			}
			if( $this->ID != $blog_forums_ID && $kind != 'forum' )
			{ // Don't install these Sidebar widgets for blog 'Forums'
				$this->include_widget( 'Sidebar', 'coll_avatar', 'core', 20 );
				if( $this->ID > $blog_a_ID )
				{
					$this->include_widget( 'Sidebar', 'evo_Calr', 'plugin', 30 );
				}
				$this->include_widget( 'Sidebar', 'coll_longdesc', 'core', 40, array( 'title' => '$title$' ) );
				$this->include_widget( 'Sidebar', 'coll_search_form', 'core', 50 );
				$this->include_widget( 'Sidebar', 'coll_category_list', 'core', 60 );

				if( $this->ID == $blog_home_ID )
				{ // Advertisements, Install only for blog #1 home blog
					...
					$this->include_widget( 'Sidebar', 'coll_item_list', 'core', 70, array(
				...
				if( $this->ID != $blog_b_ID )
				{
					$this->include_widget( 'Sidebar', 'coll_media_index', 'core', 80, 'a:11:{s:5:"title";s:12:"Random photo";s:10:"thumb_size";s:11:"fit-160x120";s:12:"thumb_layout";s:4:"grid";s:12:"grid_nb_cols";s:1:"1";s:5:"limit";s:1:"1";s:8:"order_by";s:4:"RAND";s:9:"order_dir";s:3:"ASC";'.$default_blog_param.'s:11:"widget_name";s:12:"Random photo";s:16:"widget_css_class";s:0:"";s:9:"widget_ID";s:0:"";}' );
				}
				if( ! empty( $blog_home_ID ) && ( $this->ID == $blog_a_ID || $this->ID == $blog_b_ID ) )
				{
					$sidebar_type_ID = $DB->get_var( 'SELECT ityp_ID FROM T_items__type WHERE ityp_name = "Sidebar link"' );
					$this->include_widget( 'Sidebar', 'coll_item_list', 'core', 90, array(
			...
			if( $kind == 'forum' )
			{
				$this->include_widget( 'Sidebar', 'user_avatars', 'core', 90, 'a:13:{s:5:"title";s:17:"Most Active Users";s:10:"thumb_size";s:14:"crop-top-80x80";s:12:"thumb_layout";s:4:"flow";s:12:"grid_nb_cols";s:1:"1";s:5:"limit";s:1:"6";s:9:"bubbletip";i:1;s:8:"order_by";s:8:"numposts";s:5:"style";s:6:"simple";s:6:"gender";s:3:"any";s:8:"location";s:3:"any";s:16:"widget_css_class";s:0:"";s:9:"widget_ID";s:0:"";s:16:"allow_blockcache";i:0;}' );
			}
			$this->include_widget( 'Sidebar', 'coll_xml_feeds', 'core', 100 );
			$this->include_widget( 'Sidebar', 'mobile_skin_switcher', 'core', 110 );
		}
		...
		if( $kind != 'forum' )
		{
			$this->include_widget( 'Sidebar 2', 'coll_post_list', 'core', 1 );
			if( $this->ID == $blog_b_ID )
			{
				$this->include_widget( 'Sidebar 2', 'coll_item_list', 'core', 5, array(
						'title'                => 'Sidebar links',
						'order_by'             => 'RAND',
						'item_title_link_type' => 'auto',
						'item_type_usage'      => 'special',
			...
			$this->include_widget( 'Sidebar 2', 'coll_comment_list', 'core', 10 );
			$this->include_widget( 'Sidebar 2', 'coll_media_index', 'core', 15, 'a:11:{s:5:"title";s:13:"Recent photos";s:10:"thumb_size";s:10:"crop-80x80";s:12:"thumb_layout";s:4:"flow";s:12:"grid_nb_cols";s:1:"3";s:5:"limit";s:1:"9";s:8:"order_by";s:9:"datestart";s:9:"order_dir";s:4:"DESC";'.$default_blog_param.'s:11:"widget_name";s:11:"Photo index";s:16:"widget_css_class";s:0:"";s:9:"widget_ID";s:0:"";}' );
			$this->include_widget( 'Sidebar 2', 'free_html', 'core', 20, 'a:5:{s:5:"title";s:9:"Sidebar 2";s:7:"content";s:162:"This is the "Sidebar 2" container. You can place any widget you like in here. In the evo toolbar at the top of this page, select "Customize", then "Blog Widgets".";s:11:"widget_name";s:9:"Free HTML";s:16:"widget_css_class";s:0:"";s:9:"widget_ID";s:0:"";}' );
		}
		...
		{ // Display blog title and tagline for main blogs
			$this->include_widget( 'Front Page Main Area', 'coll_title', 'core', 1 );
			$this->include_widget( 'Front Page Main Area', 'coll_tagline', 'core', 2 );
		}
		...
		$this->include_widget( 'Front Page Main Area', 'coll_featured_intro', 'core', 10, $featured_intro_params );

		if( $kind == 'main' )
		{ // Add user links widget only for main kind blogs
			$this->include_widget( 'Front Page Main Area', 'user_links', 'core', 15 );
		}
		...
		$this->include_widget( 'Front Page Main Area', 'coll_featured_posts', 'core', 20, $post_list_params );

		$this->include_widget( 'Front Page Main Area', 'coll_post_list', 'core', 25, array( 'title' => T_('More Posts'), 'featured' => 'other' ) );

		if( $kind != 'main' )
		{ // Don't install the "Recent Commnets" widget for Main blogs
			$this->include_widget( 'Front Page Main Area', 'coll_comment_list', 'core', 30 );
		}

		if( $this->ID == $blog_b_ID )
		{	// Install widget "Poll" only for Blog B on install:
			$this->include_widget( 'Front Page Main Area', 'poll', 'core', 40, array( 'poll_ID' => 1 ) );
		}
		...
		{	// Install the "Organization Members" widget only for Main collections:
			$this->include_widget( 'Front Page Secondary Area', 'org_members', 'core', 10 );
		}
		$this->include_widget( 'Front Page Secondary Area', 'coll_flagged_list', 'core', 20 );


		/* 404 Page */
		$this->include_widget( '404 Page', 'page_404_not_found', 'core', 10 );
		$this->include_widget( '404 Page', 'coll_search_form', 'core', 20 );
		$this->include_widget( '404 Page', 'coll_tag_cloud', 'core', 30 );


		/* Mobile Footer */
		$this->include_widget( 'Mobile: Footer', 'coll_longdesc', 'core', 10 );
		$this->include_widget( 'Mobile: Footer', 'mobile_skin_switcher', 'core', 20 );


		/* Mobile Navigation Menu */
		$this->include_widget( 'Mobile: Navigation Menu', 'coll_page_list', 'core', 10 );
		$this->include_widget( 'Mobile: Navigation Menu', 'menu_link', 'core', 20, array( 'link_type' => 'ownercontact' ) );
		$this->include_widget( 'Mobile: Navigation Menu', 'menu_link', 'core', 30, array( 'link_type' => 'home' ) );
		if( $kind == 'forum' )
		{ // Add menu with User Directory
			$this->include_widget( 'Mobile: Navigation Menu', 'menu_link', 'core', 40, array( 'link_type' => 'users' ) );
		}


		/* Mobile Tools Menu */
		$this->include_widget( 'Mobile: Tools Menu', 'menu_link', 'core', 10, array( 'link_type' => 'login' ) );
		$this->include_widget( 'Mobile: Tools Menu', 'msg_menu_link', 'core', 20, array( 'link_type' => 'messages' ) );
		$this->include_widget( 'Mobile: Tools Menu', 'msg_menu_link', 'core', 30, array( 'link_type' => 'contacts', 'show_badge' => 0 ) );
		$this->include_widget( 'Mobile: Tools Menu', 'menu_link', 'core', 50, array( 'link_type' => 'logout' ) );


		// Check if there are widgets to create
		if( ! empty( $this->widgets_insert_records ) )
		{ // Insert the widget records by single SQL query
			$DB->query( 'INSERT INTO T_widget( wi_coll_ID, wi_sco_name, wi_order, wi_enabled, wi_type, wi_code, wi_params ) '
								 .'VALUES '.implode( ', ', $this->widgets_insert_records ) );
		}
	}
}
