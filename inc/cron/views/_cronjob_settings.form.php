	...
	$Form->begin_fieldset( $cron_job_name.cron_job_manual_link( $cron_job_key ) );
		...
		switch( $cron_job_key )
		{
			...
			case 'send-inactive-account-reminders':
				$Form->duration_input( 'inactive_account_reminder_threshold', $Settings->get( 'inactive_account_reminder_threshold' ), T_('Trigger every'), 'days', 'minutes', array( 'note' => T_('An inactive account is an account that has been activated but the user hasn\'t connected for an extended period.') ) );
				break;
