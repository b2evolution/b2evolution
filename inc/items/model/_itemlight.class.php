...
class ItemLight extends DataObject
{
	...
	/**
	 * Cache array for slugs of the Item
	 *
	 * @var array
	 */
	var $slugs;
	...
	function get_single_url( $allow_redir = true, $blogurl = '', $glue = '&amp;', $blog_ID = NULL, $cat_ID = NULL )
	{
		...
		$item_canonical_slug = $this->get_canonical_slug();
		if( ! empty( $item_canonical_slug ) && $single_links != 'param_num' )
		{	// We can and we want to use the url title:
			$urlparam = 'title='.$item_canonical_slug;
			$urltail = $item_canonical_slug;
		}
		else
		{
			$urlparam = 'p='.$this->ID;
		...
		switch( $single_links )
		{
			...
			case 'chapters':
				...
				$chapter_url = ( $single_links == 'subchap' ? $url_Chapter->get_canonical_slug().'/' : $url_Chapter->get_url_path() );
    ...
    * Used by Itel->get_item_url()
	...
	function get_slugs( $separator = ', ' )
	{
		...
		{	// Use new slugs from the submitted form:
			$this->slugs = array_map( 'utf8_trim', explode( ',', $this->new_slugs ) );
			return implode( $separator, $this->slugs );
		}

		if( empty( $this->ID ) )
		{	// New creating Item has no slug:
			return '';
		}

		if( $this->slugs === NULL )
		{	// Get slugs from DB once and store in cache array:
			global $DB;
			$SQL = new SQL( 'Get slugs of the Item #'.$this->ID );
			$SQL->SELECT( 'slug_title, IF( slug_ID = '.intval( $this->get( 'canonical_slug_ID' ) ).', 0, slug_ID ) AS slug_order_num' );
			$SQL->FROM( 'T_slug' );
			$SQL->WHERE( 'slug_itm_ID = '.$DB->quote( $this->ID ) );
			$SQL->WHERE_and( 'slug_type = "item"' );
			if( ! empty( $this->tiny_slug_ID ) )
			{	// Exclude tiny slug from list:
				$SQL->WHERE_and( 'slug_ID != '.$DB->quote( $this->get( 'tiny_slug_ID' ) ) );
			}
			$SQL->ORDER_BY( 'slug_order_num' );
			$this->slugs = $DB->get_col( $SQL->get(), 0, $SQL->title );
		}
	...
	/**
	 * Get canonical slug of this Item
	 *
	 * @return string Canonical slug title
	 */
	function get_canonical_slug()
	{
		// Load all slugs from cache or initialize them into cache for further using:
		$this->get_slugs();

		if( ! isset( $this->slugs[0] ) )
		{	// If this Item has no canonical slug by some unknown reason:
			debug_die( 'Item #'.$this->ID.' has no canonical slug!' );
		}

		// First slug is canonical:
		return $this->slugs[0];
	}
	...
	function get( $parname )
	{
		...
		{
			case 'urltitle':
				// This param is deprecated; use $this->get_canonical_slug() instead:
				return $this->get_canonical_slug();
	...
	function set( $parname, $parvalue, $make_null = false )
	{
		switch( $parname )
		{
			...
			case 'urltitle':
				// This param is deprecated; The value is stored in DB table T_slug.
				// Set only this temp param which is used to update slugs to new by function $this->update_slugs():
				$this->new_slugs = $parvalue;
				return false;
