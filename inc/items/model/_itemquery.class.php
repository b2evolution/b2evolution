	...
	function where_ID( $p = '', $slug = '' )
	{
		...
		if( ! empty( $slug ) )
		{
			...
			{	// Starts with MINUS sign:
				...
				$this->WHERE_and( 'post_ID NOT IN ( SELECT slug_itm_ID FROM T_slug WHERE slug_title = '.$DB->quote( $this->slug ).' )' );
			}
			else
			{
				$this->FROM_add( 'LEFT JOIN T_slug ON slug_itm_ID = post_ID AND slug_type = "item" AND slug_title = '.$DB->quote( $this->slug ) );
