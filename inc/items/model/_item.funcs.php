...
/**
 * Get all custom fields by item ID with loading them into global cache array $cache_item_custom_fields
 *
 * @param integer Item ID
 * @param array Custom fields, each array item is array with keys: ID, ityp_ID, label, name, type, order, note, value.
 */
function get_item_custom_fields( $item_ID )
{
	global $cache_item_custom_fields;

	if( ! is_array( $cache_item_custom_fields ) )
	{	// Initialize array to store custom fields of Items by ID:
		$cache_item_custom_fields = array();
	}

	if( ! isset( $cache_item_custom_fields[ $item_ID ] ) )
	{	// Load item custom fields only once per Item:
		global $DB;

		$SQL = new SQL( 'Load all custom field values of Item #'.$item_ID );
		...
		$SQL->FROM_add( 'LEFT JOIN T_items__item_settings ON post_ID = iset_item_ID AND itcf_name = SUBSTRING( iset_name, 8 )' );
		$SQL->WHERE( 'post_ID = '.$DB->quote( $item_ID ) );
		$SQL->ORDER_BY( 'itcf_order, itcf_ID' );
		$custom_fields = $DB->get_results( $SQL->get(), ARRAY_A, $SQL->title );

		$cache_item_custom_fields[ $item_ID ] = array();
		foreach( $custom_fields as $c => $custom_field )
		{	// Make field name/code as key/index of array:
			$cache_item_custom_fields[ $item_ID ][ $custom_field['name'] ] = $custom_field;
		}
	}

	return $cache_item_custom_fields[ $item_ID ];
}
 ...
 *
 * @param integer Item ID
 * @param string Field index which by default is the field name, see {@link get_item_custom_fields()}
 ...
 */
function get_item_custom_field_value( $item_ID, $field_index, $restrict_type = false )
{
	// Get all custom fields by item ID:
	$custom_fields = get_item_custom_fields( $item_ID );
...
/**
 * Render all custom fields of Item to HTML code
 *
 * @param integer Item ID
 * @param array Params
 * @return string
 */
function render_item_custom_fields( $item_ID, $params = array() )
{
	// Make sure we are not missing any param:
	$params = array_merge( array(
