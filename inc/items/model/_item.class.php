...
class Item extends ItemLight
{
	...
	function load_from_Request( $editing = false, $creating = false )
	{
		...
		$slugs_param_name = 'post_slugs';
		if( $post_slugs === NULL )
		{	// Try to get slugs from deprecated param which still may be used by 3rd party skins from disp=edit:
			$post_slugs = param( 'post_urltitle', 'string', NULL );
			$slugs_param_name = 'post_urltitle';
		}
		if( $post_slugs !== NULL )
		{
			$this->new_slugs = $post_slugs;
			...
			$post_urltitle = explode( ',', $post_slugs );
			foreach( $post_urltitle as $u => $slug_urltitle )
			{
				...
				{	// Display error if one of item slugs doesn't contain at least 1 non-numeric character:
					param_error( $slugs_param_name, T_('All slugs must contain at least 1 non-numeric character.') );
				}
			}
	...
	function dbinsert( $display_restrict_status_messages = true )
	{
		...
		{ // We could insert the item object..
			...
			$new_slugs = utf8_trim( $this->new_slugs );
			if( empty( $new_slugs ) )
			{	// Use autogenerated slug title if new slugs have not been entered on the submitted form:
				$this->new_slugs = urltitle_validate( '', $this->get( 'title' ), $this->ID, false, 'slug_title', 'slug_itm_ID', 'T_slug', $this->get( 'locale' ) );
			}
			...
			if( $result && ( $result = $new_tiny_Slug->dbinsert() ) )
			{
	 ...
	 * @private
	 * 
	 * @param string Slug title - DEPRECATED
	 * @return array Slug objects
	 */
	function update_slugs( $dummy = NULL )
	{
		if( empty( $this->ID ) )
		{	// If item isn't created:
			debug_die( 'Item must be inserted in DB before slug updating!' );
		}

		$new_slugs = utf8_trim( $this->new_slugs );
		if( empty( $new_slugs ) )
		{	// No new slugs to update:
			return;
		}
		...
		$new_slugs = explode( ',', $new_slugs );

		$s = 0;
		foreach( $new_slugs as $new_slug_title )
		{
			// Replace special chars/umlauts:
			$new_slug_title = replace_special_chars( utf8_trim( $new_slug_title ), $this->get( 'locale' ) );
			...
			// Try to get existing slug from DB by title:
			$item_Slug = & $SlugCache->get_by_name( $new_slug_title, false, false );

			if( ! $item_Slug || $item_Slug->get( 'itm_ID' ) != $this->ID )
			{	// Create new slug if it doesn't exist in DB or slug is used for other object(Chapter or Item):
				$item_Slug = new Slug();
				$item_Slug->set( 'type', 'item' );
				$item_Slug->set( 'itm_ID', $this->ID );
				$item_Slug->set( 'title', urltitle_validate( $new_slug_title, $new_slug_title, $this->ID, false, $item_Slug->dbprefix.'title', $item_Slug->dbprefix.'itm_ID', $item_Slug->dbtablename, $this->get( 'locale' ) ) );
				$item_Slug->dbinsert();
			}

			if( $s == 0 && // First slug must be used as canonical for this Item
			    ( ! $canonical_Slug || // If this Item has no canonical slug yet OR
			      $this->get( 'canonical_slug_ID' ) != $item_Slug->ID // If other was used as canonical slug
			    ) )
			{	// Update canonical slug of this Item:
				$this->set( 'canonical_slug_ID', $item_Slug->ID );
				parent::dbupdate();
			}
		...
		// Reset cached slugs to get the updated from DB:
		$this->new_slugs = NULL;
		$this->slugs = NULL;
	}
