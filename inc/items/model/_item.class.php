...
class Item extends ItemLight
{
	...
	function dbupdate( $auto_track_modification = true, $update_slug = true, $dummy = true )
	{
		...
		{
			if( ! isset( $dbchanges['last_touched_mts'] ) )
...
// fp>yura: please check why this function is very different from get_content_excerpt() and use best code for both
	...
	/**
	 * Update field last_touched_mts and parent categories
	 *
	...
	/**
	 * Update field itud_read_item_mts for current User
	 *
	...
	function update_read_timestamps( $read_post = true, $read_comments = true )
	{
		...
		global $DB, $current_User, $localtimenow, $localmicrotimenow, $user_post_read_statuses;
		...
		{	// Update the read status:
			...
			{	// Update a post read timestamp:
				$update_fields = 'itud_read_item_mts = '.$DB->quote( $localmicrotimenow );
			}
			if( $read_comments )
			{	// Update a comments read timestamp:
				...
				$update_fields .= 'itud_read_comments_mts = '.$DB->quote( $localmicrotimenow );
			}
		...
		{	// Insert new read status:
			...
			if( $read_post )
			{	// Update a post read timestamp:
				$insert_fields = 'itud_read_item_mts';
			...
			{	// Update a comments read timestamp:
				...
				$insert_fields .= 'itud_read_comments_mts';
				$insert_values .= $DB->quote( $localmicrotimenow );
			}
		...
		$this->set_user_data( 'item_date', $localmicrotimenow );
	}
	...
	function get_user_data( $field = NULL )
	{
		...
		{ // Get the read post date only one time from DB and store it in cache array
			...
			$SQL->SELECT( 'IFNULL( itud_read_item_mts, 0 ) AS item_date, IFNULL( itud_read_comments_mts, 0 ) AS comments_date, itud_flagged_item AS item_flag' );
