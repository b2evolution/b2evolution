...
class Item extends ItemLight
{
	...
	function load_from_Request( $editing = false, $creating = false )
	{
		...
		{ // The form has sent an excerpt field:
			if( $post_excerpt != $this->excerpt )
			{	// The post excerpt has been edited by the user, so it means it is no longer auto-generated:
...
// Therefore, I think we should deprecate these 2 params. And maybe we should bring back the original function get_excerpt() (depends on what it was exactly)
	 ...
	 *
	 * This function never uses the user entered excerpt.
	 *
	 ...
	 * @return string
	 */
// fp>yura: please check why this function is very different from get_autogenerated_excerpt() and use best code for both
...
// fp>yura: I think we should remove return above and add the following code:
/*				if( $this->excerpt_autogenerated )
				{	// As far as we know, we are still auto-generating excerpts for this Item at this moment, 
					...
					// fp> this may have the additional positive effect of handling the case where we manually edit excerpt by making it identical to new autogenerated excerpt 
					//		(the received "new excerpt" will be equal to $this->exceprt)
...
// fp>yura: we should probably use the following so that anything (like XML-RPC) that tries to empty excerpt, puts us back on auto-generation.
/*
			case 'excerpt':
				if( empty( $parvalue ) )
				{	// We have are trying to make the excerpt empty.
					// This means we want to go back to autogenerated excerpt:
...
// fp>yura: I think we can remove this after we generate excerpt in this->set( 'content' ...
		if( $this->excerpt_autogenerated )
	 ...
	 * @return boolean true on success
	 */
// fp>yura: check if we can remove the last param (but in this case, change it to $dummy in case a plugin uses this)
...
// fp>yura, maybe we can remove this if we generate in this->set('content' ...  (the "if( empty( $this->excerpt )" part will be be handled in this->set('excerpt'... )
		if( $update_excerpt )
		{ // We want to update the excerpt:
			...
			{ // Item excerpt is empty or the content was changed and the excerpt is autogenerated:
				$db_changed = $this->update_excerpt();
			}
		}
	 ...
	 */
// fp>yura: please see comments below, maybe we can remove this function completely
	function update_excerpt( $maxlen = 254, $tail = '&hellip;' )
	{
		// Clear prerendered content to generate excerpt from new content:
// fp>yura: there are many other places where we call get_autogenerated_excerpt() withtout a previous delete_prerendered_content()
// It seems suspicious that we need it here and not elsewhere. I think either we need it everywhere or we don't need at all. Please check.
		$this->delete_prerendered_content();
		...
		if( ! empty( $excerpt ) )
// fp>yura: why do we discard an empty excerpt? If content is made empty, autogenerated excerpt should also become empty (otherwise we would keep outdated/wrong content in the excerpt :/ )
		{	// We have something to act as an excerpt...
			if( $excerpt_is_updated = $this->set( 'excerpt', $excerpt ) )
			{
// fp>yura: why do we need to set this?
				$this->set( 'excerpt_autogenerated', 1 );
			}
			return $excerpt_is_updated;
		}
