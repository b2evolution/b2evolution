		...
		for( $i = 1 ; $i <= $custom_field_count; $i++ )
		{
			...
			if( $custom_field_data['type'] == 'computed' )
			{
				...
				{	// Formula must be not empty:
					$Messages->add( sprintf( T_('Please enter formula for computed custom field "%s".'), $custom_field_data['label'] ) );
				}
				...
				{	// Inform once user about changed formula:
					$Messages->add( T_('You changed one or several formulas. All posts need to be re-saved to update the results of these formulas.'), 'warning' );
		...
		foreach( $custom_fields as $custom_field )
		{
			...
			{	// If formula has at least one field:
				foreach( $formula_fields[1] as $formula_field )
				{
					...
					{	// Not found field:
						$Messages->add( sprintf( T_('You use a not recognized field %s in the formula %s of the field "%s".'),
					...
					{	// Field with wrong type is used in formula:
						$Messages->add( sprintf( T_('Only numeric or computed field can be used in formula, please remove field %s from the formula %s of the field "%s".'),
								'<code>'.$formula_field.'</code>',
								'<code>'.$custom_field['formula'].'</code>', 
								$custom_field['label']
							), 'warning' );
			...
			{	// Check for correct formula:
				...
				{	// Display warning when formula cannot be executed properly:
					$Messages->add( sprintf( T_('Please check formula %s of the field "%s" because it is wrong and cannot be executed properly.'),
							'<code>'.$custom_field['formula'].'</code>',
							$custom_field['label']
						), 'warning' );
				}
