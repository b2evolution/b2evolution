...
class geoip_plugin extends Plugin
{
	...
	function GetDefaultSettings( & $params )
	{
		...
		return array(
			'download_url' => array(
				'label' => T_('DB Download URL'),
				'note' => sprintf( T_('Enter URL of %s for download'), '<code>GeoLite2-Country....tar.gz</code>' ),
				'size' => 100,
				),
			'datfile' => array(
...
<?php
	...
	function download_geoip_data()
	{
		...
		{	// If "DB Download URL" is not defined:
			$this->print_tool_log( sprintf( T_('Enter URL of %s for download'), '<code>GeoLite2-Country....tar.gz</code>' ), 'error' );
			return;
		}
