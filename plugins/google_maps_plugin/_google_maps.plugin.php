	...
	function SkinTag( & $params )
	{
		...
		{	// Display message if google API key is not defined:
			...
			echo '<div class="evo_plugin__google_maps_no_key">'
					...
					.'<div><div>'.sprintf( T_('You must specify a valid Google Maps API key in the Plugins settings <a %s>Collection Settings</a> tab to use the plugin.'), 'href="'.$url.'"' ).'</div></div>'
				.'</div>';
			return;
		}
		...
		{ // Collection list widget
			...
			$sql = 'SELECT post_ID, cat_blog_ID,
							...
							FROM T_items__item
							LEFT JOIN T_items__item_settings ON iset_item_ID = post_ID AND iset_name IN ( "latitude", "longitude" )
							LEFT JOIN T_postcats ON postcat_post_ID = post_ID
							LEFT JOIN T_categories ON cat_ID = postcat_cat_ID
							WHERE
								( iset_value IS NOT NULL OR post_ctry_ID IS NOT NULL OR post_rgn_ID IS NOT NULL OR post_subrg_ID IS NOT NULL OR post_city_ID IS NOT NULL )
								AND '.$Blog->get_sql_where_aggregate_coll_IDs( 'cat_blog_ID', $blog_ID ).
							' GROUP BY post_ID, cat_blog_ID ORDER BY '.$order_by.' LIMIT '.$limit;

			$items = $DB->get_results( $sql, ARRAY_A, 'Fetching posts with location information' );
