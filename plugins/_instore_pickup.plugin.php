 ...
 * @package plugins
 */
class instore_pickup_plugin extends Plugin
{

	var $name;
	var $code = 'ship_instore';
	var $priority = 10;
	var $version = '7.0.0';
	var $author = 'b2evolution Group';
	var $group = 'shipment';

	var $default_shipping_costs = "[USD]0.00\n[EUR]0.00\n";
	...
	{
		$this->name  = T_('In-store pick-up');
		$this->short_desc = $this->T_('In-store pick-up shipment plugin');
	}
	...
	function get_coll_setting_definitions( & $params )
	{
		...
		$plugin_params['coll_shipping_costs'] = array(
			'label' => $this->T_('Shipping cost'),
			'type' => 'textarea',
			'note' => $this->T_('Format: [<code>Currency code</code>] <code>Pickup cost</code>'),
			'rows' => 10,
			'cols' => 60,
			'defaultvalue' => $this->default_shipping_costs
		);
	...
	function GetShippingCost( & $params )
	{
		...
		{
			return $costs[$current_Currency->code];
		}
	...
	function parse_shipping_cost()
	{
		...
		$shipping_costs = $this->get_coll_setting( 'coll_shipping_costs', $Blog );
