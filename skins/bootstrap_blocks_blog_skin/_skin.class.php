...
class bootstrap_blocks_blog_Skin extends Skin
{
	...
	function display_init()
	{
		...
		foreach( $this->get_screen_sizes() as $screen_key => $screen )
		{
			...
			// Block height:
			$this->dynamic_style_rule( $screen_key.'_block_height', '.bbbs_posts_list>div>.bbbs_post_block { height: $setting_value$px }', array(
					'check' => 'not_empty'
			) );
			// Cover height:
			$this->dynamic_style_rule( $screen_key.'_cover_height', '.bbbs_posts_list .bbbs_post_cover { height: $setting_value$px }', array(
					'check' => 'not_empty'
			) );
			if( isset( $screen['min_width'] ) )
