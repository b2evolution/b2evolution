...
*/
	...
	function display_button_recent_topics()
	{
		...
		{	// For logged in users:
			...
			$unread_posts_SQL->FROM_add( 'LEFT JOIN T_users__item_read_status ON post_ID = uirs_item_ID AND uirs_user_ID = '.$DB->quote( $current_User->ID ) );
			...
			$unread_posts_SQL->WHERE_and( 'post_last_touched_mts > '.$DB->quote( date2mysql( $localtimenow - 30 * 86400 ) ) );
			// In theory, it would be more safe to use this comparison:
			// $unread_posts_SQL->WHERE_and( 'uirs_item_ID IS NULL OR uirs_read_item_mts <= post_last_touched_mts' );
