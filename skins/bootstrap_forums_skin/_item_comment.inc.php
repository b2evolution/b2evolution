...
<?php echo $params['comment_block_start'];
	...
	<tr<?php echo $comment_class; ?> id="comment_<?php echo $Comment->ID; ?>">
		<td class="ft_avatar valign-top"><?php
		$Comment->author2( array(
				'before'       => '',
				'after'        => '',
				'before_user'  => '',
	...
	<?php
		...
		{ // Display action buttons only for existing comments(Disable for previewed comment)
			...
			{ // Display button to quote this comment
				echo '<a href="'.$commented_Item->get_permanent_url().'?mode=quote&amp;qc='.$Comment->ID.'#form_p'.$commented_Item->ID.'" title="'.T_('Reply with quote').'" class="'.button_class( 'text' ).' floatleft quote_button">'.get_icon( 'comments', 'imgtag', array( 'title' => T_('Reply with quote') ) ).' '.T_('Quote').'</a>';
			}
