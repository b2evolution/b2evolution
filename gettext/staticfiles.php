...
/**
 * Callback function to translate string by POFile object
 *
 * @param array Matches
 * @return string Translated function
 */
function pofile_translate_callback( $matches )
{
	global $POFile;

	if( empty( $POFile ) || empty( $matches[1] ) )
	{
		return '';
	}

	return $POFile->translate( preg_replace( '|[\n\t\r]+|', ' ', stripslashes( $matches[1] ) ) );
}
	...
	case 'merge':
		...
		{ // loop targets/locales
			...
			{ // loop through sourcefiles
				...
				$path_to_root = '';
				for($i = 1; $i < count(explode('/', $srcfile)); $i++)
				{
				...
				$replace = array(
					...
					$target,
					$charset,
					'<!-- This file was generated automatically by /gettext/staticfiles.php - Do not edit this file manually -->'.$lineend.'<html'
				);
				...
				{ // translate everything
					$text = preg_replace_callback( '/'.TRANSTAG_OPEN.'(.*?)'.TRANSTAG_CLOSE.'/s', 'pofile_translate_callback' , $text );
