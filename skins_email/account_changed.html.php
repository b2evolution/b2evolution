...
if( $User = & $UserCache->get_by_ID( $params['user_ID'], false, false ) )
{
	...
	{ // If profile pictre has been changed
		...
		echo '<p'.emailskin_style( '.p' ).'>'.$User->get_avatar_File()->get_tag( '', '', '', '', 'fit-320x320','original', '', '', '', '', '', '#', '', 1, NULL, array( 'width', 'height' ) ).'</p>'."\n";
	}
	...
	{ // Display the newly uploaded file only if it was not set as main profile picture
		...
		echo '<p'.emailskin_style( '.p' ).'>'.$new_File->get_tag( '', '', '', '', 'fit-320x320','original', '', '', '', '', '', '#', '', 1, NULL, array( 'width', 'height' ) ).'</p>'."\n";
	}
